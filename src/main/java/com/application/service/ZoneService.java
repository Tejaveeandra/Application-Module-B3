...
public class ZoneService {
    ...
    public List<EmployeesDto> getEmployeesByZone(int zoneId, String category) {
        ...
        // Apply category filter if provided (case-insensitive) - filter at ZonalAccountant level
        if (category != null && !category.trim().isEmpty()) {
            String categoryLower = category.trim().toLowerCase();
            activeAccountants = activeAccountants.stream()
                    .filter(accountant -> {
                        // Check if campus and business type exist
                        if (accountant.getCampus() == null ||
                            accountant.getCampus().getBusinessType() == null) {
                            return false;
                        }
                       
                        String businessTypeName = accountant.getCampus().getBusinessType()
                                .getBusinessTypeName().toLowerCase();
    ...
    private void rebuildBalancesFromDistributions(int empId, int acYearId, int typeId, int createdBy, Float amount) {
        ...
        System.out.println("DEBUG: rebuildBalancesFromDistributions - Employee: " + empId + ", Received Distributions: " + received.size());
        ...
        System.out.println("DEBUG: rebuildBalancesFromDistributions - Employee: " + empId + ", Given Away Distributions: " + givenAway.size());
...
//      dto.setIssueDate(dist.getIssueDate());
    ...
    /**
     * Validates that the sender (createdBy) actually owns the requested application range.
